          ...
          lines.forEach((line, index) => {
            if (!closestPointFound) {
              if (distance(actualPoint, line.point_1) < minTwoPointsDistance) {
                line.point_1.x = actualPoint.x;
                line.point_1.y = actualPoint.y;
                closestPointFound = true;
              } else if (
                distance(actualPoint, line.point_2) < minTwoPointsDistance
              ) {
                line.point_2.x = actualPoint.x;
                line.point_2.y = actualPoint.y;
                closestPointFound = true;
              }
            }
          });
...
export function ExerciseWriter(props) {
  ...
  return (
    <selectedShapesContext.Provider
      value={{ selectedShapes, setSelectedShapes, shapes }}
    >
...
const ExamShapesBoard = React.memo((props) => {
  ...
  const { selectedShapes, setSelectedShapes } = useContext(
  ...
  const handleSubmit = (event) => {
    ...
    const normalizedShapes = shapes.map((shape) => {
      ...
      lines = [...lines]
        ...
        .map((line) => {
          const [point_1, point_2] = [
            normalizePoint(line.point_1, paperSize),
            normalizePoint(line.point_2, paperSize),
          ];
          return { point_1, point_2 };
        });
      return lines;
  ...
  return (
    <div className="exam-writer-paper">
      <svg ref={paperRef} className="exam-writer-draw-paper">
        {shapes.map((shape, index) => {
          ...
          return (
            <g
              data-index={index}
              data-center={JSON.stringify(shape.center)}
              data-action={action.value}
              key={JSON.stringify(shape) + index}
              fill="blue"
              transform={`rotate(${rotateDeg},${
                (center.x + translate.x) * scale.x
              },${(center.y + translate.y) * scale.y}) translate(${
                translate.x
              },${translate.y}) scale(${scale.x},${scale.y})`}
              className=" graph"
            >
              <rect
                x={container.x}
              ...
              {lines.map((line) => {
                return (
                  <g key={JSON.stringify(line) + index} className="graph">
                    <line
                      x1={line.point_1.x}
                      y1={line.point_1.y}
                      x2={line.point_2.x}
      ...
      <div className="exam-writer-paper-tools">
        <button onClick={handleActionClick} name="delete">
          delete shape
        </button>
        <button onClick={handleActionClick} name="translate">
          move shape
        </button>
        <button onClick={handleActionClick} name="rotate">
          rotate shape
        </button>
...
function ShapeImage(props) {
  ...
  return (
    <svg
      ref={imageRef}
      className="exam-writer-shape-image"
      index={props.index}
      onClick={() =>
        setSelectedShapes((prev) => [...prev, shapes[props.index]])
      }
    >
